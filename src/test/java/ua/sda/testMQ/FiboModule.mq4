...
int Buffer[];  // Объявление массивов (под буферы индикатора)
...
int start()
  {
   int i_bars,// Индекс бара
    counted_bars;// Количество просчитанных баров
    counted_bars=IndicatorCounted(); // Количество просчитанных баров
     i=Bars-Counted_bars-1; // Индекс первого непосчитанного
      while(i_bars>=0)                      // Цикл по непосчитанным барам
          {
           Bufffer[i]=i;                 // Значение 0 буфера на i-ом баре
...
Print("return nonSymm globalHighTimeCurrent = ", globalHighTimeCurrent, "globalLowTimeCurrent", globalLowTimeCurrent);
   ...
   datetime highTime = Time[localHighTimeCurrent];
   datetime lowTime  = Time[localLowTimeCurrent];
Print("int start localHighTimeCurrent = ", localHighTimeCurrent, "localLowTimeCurrent", localLowTimeCurrent);
   if(High[localHighTimeCurrent]>Low[localLowTimeCurrent])
     {
      WindowRedraw();
      ObjectCreate(MPrefix+"FIBO_MOD",OBJ_FIBO,0,highTime,High[localHighTimeCurrent],lowTime,Low[localLowTimeCurrent]);
      color levelColor=Red;
     }
   else
     {
      WindowRedraw();
      ObjectCreate(MPrefix+"FIBO_MOD",OBJ_FIBO,0,lowTime,Low[localLowTimeCurrent],highTime,High[localHighTimeCurrent]);
      levelColor=Green;
     }
